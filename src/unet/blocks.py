...
class EncoderBlock(nn.Module):
    ...
    """
    def __init__(self, filter_num=64, in_channels=1, dropout=False, dropout_rate=0.3, structured_dropout=False):
    ...
    def apply_manual_dropout_mask(self, x, seed):
        ...
        if self.structured_dropout is False:
            dropout_mask = torch.bernoulli(input=torch.empty(x.shape[0], x.shape[1], x.shape[2], x.shape[3]).fill_(self.dropout_rate),
                                           generator=torch.Generator().manual_seed(seed))
        else:
            dropout_mask = torch.bernoulli(input=torch.empty(x.shape[0], x.shape[1]).fill_(self.dropout_rate),
                                           generator=torch.Generator().manual_seed(seed))
            dropout_mask = dropout_mask.unsqueeze(dim=-1).unsqueez(dim=-1)
...
class DecoderBlock(nn.Module):
    ...
    """
    def __init__(self, in_channels, concat_layer_depth, filter_num, interpolate=False, dropout=False, dropout_rate=0.3, structured_dropout=False):
